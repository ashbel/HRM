...
namespace Hrm_SystemCore.Controllers
{
    ...
    public class TrainingController : Controller
    {
        ...
        public TrainingController(HRMEntities db)
        {
            this.db = db;
        }
        ...
        {
            var tblTrainings = db.tblTrainings.Include(t => t.tblCours).Include(t => t.tblEmployee).ToList();
            ...
            return View(tblTrainings.OrderBy(c => c.tr_id));
        }
        ...
        public ActionResult EnrollUsers(FormCollection formCollection)
        {
            if (Request != null)
            {
                var employee = formCollection["emp_id"].ToString();
                var department = formCollection["dpt_id"].ToString();
                var position = formCollection["pos_id"].ToString();
                var branch = formCollection["br_id"].ToString();
                var course = Convert.ToInt32(formCollection["course"]);
        ...
        public ActionResult Create(tblTraining tblTraining)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit(tblTraining tblTraining)
        {
            if (ModelState.IsValid)
            {
