...
namespace Hrm_SystemCore.Controllers
{
    ...
    public class EmployeeController : Controller
    {
        ...
        public EmployeeController(IWebHostEnvironment hostingEnvironment, HRMEntities db)
        {
        ...
        {
            var tblemployees = await db.tblEmployees.Include(t => t.tblBranch).Include(t => t.tblDepartment).Include(t => t.tblEmployeeStatu).Include(t => t.tblPosition)
                .ToListAsync();
            ...
            return View(tblemployees.OrderBy(c => c.emp_id));
        ...
        public ActionResult Create(FormCollection form)
        {
            ...
            var cv = Request.Form.Files.GetFile("cv");
            var olevel = Request.Form.Files.GetFile("olevel");
            var alevel = Request.Form.Files.GetFile("alevel");
            var uni = Request.Form.Files.GetFile("uni");
            var idscan = Request.Form.Files.GetFile("idscan");
            var passport = Request.Form.Files.GetFile("passport");
            var drvlicence = Request.Form.Files.GetFile("drvlicence");
        ...
        public ActionResult UploadFile(FormCollection formCollection)
        {
            if (Request.Form.Files != null)
            {
                ...
                var path= Path.Combine(webRootPath, "../empImg/" + id);
                ...
                var tblemployee = db.tblEmployees.Find(id);
                var img = db.tblImages.FirstOrDefault(c=>c.emp_id == tblemployee.emp_id);
        ...
        public string filename (IFormFile uploadFile, int emp_id)
        {
            string fileName = "";
            ...
            {
                var webRootPath = _hostingEnvironment.WebRootPath;
                var path = Path.Combine(webRootPath, "../EmployeeFiles/Docs/" + emp_id);
        ...
        public ActionResult UploadDocuments(FormCollection formCollection)
        {
            if (Request != null)
            {
                ...
                var uploadFile = Request.Form.Files["other_doc"];
                if (Request != null)
                {
                    var webRootPath = _hostingEnvironment.WebRootPath;
                    var path = Path.Combine(webRootPath, "../EmployeeFiles/Docs/" + id);
                    if (!Directory.Exists(path))
                    {
                        Directory.CreateDirectory(path);
                    }
